...
namespace HFPS.Systems
{
    ...
    public class
    ZombieBehaviourAI
    : NPCEntity, INPCReaction, IOnAnimatorState, ISaveable
    {
#region Structures
        [System.Serializable]
        ...
        public enum SecondaryBehaviour
        {
            Normal,
            Agony,
            Scream,
...
#region Public Variables
        ...
        private PrimaryBehaviour primaryBehaviour = PrimaryBehaviour.Sleep;

        [ReadOnly, SerializeField]
        private SecondaryBehaviour
...
#region Private Variables
        ...
        void Awake()
        {
            ...
            playerHealth =
                playerController.gameObject.GetComponent<HealthManager>();
        ...
        void Start()
        {
            ...
            {
                Debug
                    .LogError("[Zombie AI] Create NavMesh first or move AI to NavMesh!");
            }
        }
        ...
        void Update()
        {
            ...
            playerHead =
                new Vector3(playerObject.position.x,
                    _playerCam.y,
            ...
            float pmagnitude =
                new Vector3(pvelocity.x, 0, pvelocity.z).magnitude;

            //Is player currently moving?
            bool playerMoving =
                DistanceOf(_playerCam) > agent.stoppingDistance &&
                pmagnitude > 1;
            ...
            ignorePlayer =
                health.Health <= sensorSettings.lowHP &&
                behaviourSettings.lowHPIgnorePlayer &&
                canIgnorePlayer;
            ...
            if (isLerpSpeed)
            {
                agent.speed =
                    Mathf
                        .MoveTowards(agent.speed,
                        targetAgentSpeed,
                        Time.deltaTime * aiSettings.speedChangeSpeed);
            }
            ...
            if (sleepBehaviour != StartingSleep.None)
            {
                ...
                {
                    Collider[] threats =
                        ThreatsInRadius(sensorSettings.threatRadius);
                    Transform newWakeThreat = GetClosestVisibleThreat(threats);

                    if (newWakeThreat != null) wakeThreat = newWakeThreat;
                }

                //Set zombie animation to a sleep behaviour
                if (
                    wakeThreat &&
                    InDistance(sensorSettings.idleHearRange,
                    wakeThreat.position)
                )
                {
                    PlaySoundRandom(m_NPCSounds.ReactionSounds,
                    m_NPCSoundsVolume.ReactionVolume);
            ...
            else if (SearchForThreat())
            {
                ...
                if (!canChasePlayer)
                {
                    ...
                    SetAgentDestination(lastThreatPosition,
                    aiSettings.runSpeed,
                    true);
                    ...
                    else if (!screamTriggered && !isReaction)
                    {
                        ...
                        enableRootPosition =
                            !behaviourSettings.runToThreat &&
                            aiSettings.walkRootMotion;
                        SetAnimatorState(!behaviourSettings.runToThreat,
                        behaviourSettings.runToThreat);
                        canChasePlayer = !secondaryPending;
                        secondaryPending =
                            isIdle || !isNPCAwake || isAgonyOrEating;
                        waitToChase = false;
                    }
                    else
                    {
                ...
                else if (!secondaryPending)
                {
                    ...
                    aiSettings.runSpeed,
                    false,
                    aiSettings.accelerateManually);

                    if (behaviourSettings.runToThreat)
                    {
                        SetAnimatorState(false,
                        shouldMove || playerMoving,
                        !shouldMove,
                        scream: false);
                    }
                    else
                    {
                        SetAnimatorState(shouldMove || playerMoving,
                        false,
                        !shouldMove,
                        scream: false);
                    }
                    ...
                    if (
                        priorityThreat &&
                        InDistance(sensorSettings.attackDistance,
                        priorityThreat.position) &&
                        IsObjectInSights(sensorSettings.attackFOV,
                        priorityThreat.position) &&
                        canAttack
                    )
                    {
                        if (!eventPlayAttackSound)
                            PlaySoundRandom(m_NPCSounds.AttackSounds,
                            m_NPCSoundsVolume.AttackVolume);
                        animator
                            .SetInteger("AttackState",
                            lastAttack =
                                Utilities
                                    .RandomUnique(0,
                                    attackAnimations,
            ...
            )
            {
                ...
                if (shouldMove && primaryBehaviour == PrimaryBehaviour.Chase)
                {
                    ...
                    )
                    {
                        ...
                        b_Patrol =
                            new Behaviour(StartCoroutine(Patrol(behaviourSettings
                                    .playerLostPatrol)));
                        secondaryPending = true;
                    }
                    else
        ...
        void SecondaryUpdate()
        {
            ...
            if (behaviourSettings.enableHunger)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (
                            closestHungerPoint &&
                            SeesObject(sensorSettings.sightsDistance,
                            closestHungerPoint.transform.position)
                        )
                        {
                            //Should we enable root motion?
                            rotateTransform = true;
                            ...
                            SetAgentDestination(closestHungerPoint
                                .transform
                                .position,
                            aiSettings.runSpeed,
                            false,
                            ...
                            agent.stoppingDistance =
                                aiSettings.eatingStoppingDist;

                            //Start Eat Behaviour Trigger
                            b_Eating =
                                new Behaviour(StartCoroutine(EatOrAgony(0,
                                    SecondaryBehaviour.Eating,
                                    true)));
            ...
            if (!shouldMove)
            {
                ...
                if (zombieBehaviour == GeneralBehaviour.Waypoint2Waypoint)
                {
                    ...
                    SetAgentDestination(goToLastWaypoint
                        ? lastWaypointPos
                        : NextWaypoint(),
                    runToDestination
                        ? aiSettings.runSpeed
                    ...
                    zombieBehaviour == GeneralBehaviour.W2WPatrol ||
                    zombieBehaviour == GeneralBehaviour.W2WPatrolIdle
                )
                {
                    if (!isWaypointSet)
                    {
        ...
        Vector3? NextWaypoint()
        {
            if (waypoints && waypoints.Waypoints.Count > 1)
            {
                ...
                {
                    Waypoint[] possibleWaypoints =
                        waypoints
                            .Waypoints
                            .Where(x =>
                                !x.isOccupied &&
                                IsPathPossible(x.transform.position) &&
                                x != nextWaypoint)
                            .ToArray();
                    ...
                    if (possibleWaypoints.Length > 0)
                    {
                        ...
                        nextWaypoint =
                            possibleWaypoints[Random
                                .Range(0, possibleWaypoints.Length - 1)];
                ...
                else
                {
                    ...
                    int next =
                        nextWaypoint != null
                            ? possibleWaypoints.IndexOf(nextWaypoint) ==
                            possibleWaypoints.Count - 1
                                ? 0
                                : possibleWaypoints.IndexOf(nextWaypoint) + 1
                            : 0;
...
#region Interface Callbacks
        public void OnStateEnter(AnimatorStateInfo state, string name)
        {
            ...
            {
                secondaryBehaviour = SecondaryBehaviour.Eating;
                StartCoroutine(EatOrAgony(state.length,
                SecondaryBehaviour.Eating,
                false));
            ...
            else if (name == "Turn")
            {
                ...
                b_Reaction =
                    new Behaviour(StartCoroutine(Reaction(state.length)));
            }
        }
        ...
        public void HitReaction()
        {
            ...
            PlaySound(m_NPCSounds.TakeDamageSound,
            m_NPCSoundsVolume.TakeDamageVolume);
        ...
        public void SoundReaction(Vector3 pos, bool closeSound)
        {
            ...
            if (
                primaryBehaviour != PrimaryBehaviour.Chase &&
                distance <= sensorSettings.soundReactRange &&
                canReact
            )
            {
                //Wake NPC
...
#region Behaviour Trees
        ...
        {
            //Play Scream Sound
            if (!eventPlayScreamSound)
                PlaySound(m_NPCSounds.ScreamSound,
                m_NPCSoundsVolume.ScreamVolume);
        ...
        IEnumerator Patrol(float time)
        {
            ...
            if (playAttractedSounds)
                PlaySoundRandom(m_NPCSounds.IdleSounds,
                m_NPCSoundsVolume.IdleVolume);
        ...
        IEnumerator
        EatOrAgony(float time, SecondaryBehaviour behaviour, bool trigger)
        {
            if (
                behaviour == SecondaryBehaviour.Agony ||
                !trigger && behaviour == SecondaryBehaviour.Eating
            )
            {
                if (behaviour == SecondaryBehaviour.Agony)
                {
                    //Play Agony Sound
                    if (!eventPlayAgonySound)
                        PlaySound(m_NPCSounds.AgonizeSound,
                        m_NPCSoundsVolume.AgonizeVolume);
            ...
            else if (behaviour == SecondaryBehaviour.Eating)
            {
                ...
                HungerPoint.HungerPoints hunger_points =
                    closestHungerPoint.GetHungerPoints();
        ...
        IEnumerator Reaction(float time)
        {
            ...
            enableRootRotation = false;
            enableRootPosition =
                !behaviourSettings.runToThreat && aiSettings.walkRootMotion;
            canReact = true;
            ...
            if (!veryCloseAttract)
            {
                ...
                behaviourSettings.runToThreat,
                scream: false);
                SetAgentDestination(reactionPosition,
                aiSettings.runSpeed,
                false,
                aiSettings.accelerateManually);
            ...
            yield return new WaitForSeconds(behaviourSettings
                        .patrolTime
                        .Random());
...
#region Other Functions
        ...
        bool SearchForThreat()
        {
            ...
            if (
                playerHealth.isDead ||
                behaviourSettings.playerInvisible ||
                ignorePlayer
            )
            {
                priorityThreat = null;
            ...
            if (priorityThreat)
            {
                sightsResult =
                    priorityThreat == playerObject
                        ...
                        : GetSightsResult(priorityThreat.position,
                        priorityThreat.position);
            ...
            if (behaviourSettings.attackOthers)
            {
                ...
                if (threats.Length > 0)
                {
                    ...
                    foreach (var threat in threats)
                    {
                        ...
                        {
                            if (
                                GetSightsResult(playerObject.position,
                                playerHead)
                            )
                            {
        ...
        Transform GetClosestVisibleThreat(Collider[] threats)
        {
            ...
            Transform[] visibleThreats =
                threats
                    .Where(x =>
                        GetSightsResult(x.transform.position,
                        x.transform.position))
                    .Where(x =>
                        !x
                            .gameObject
                            .HasComponent(out DamageBehaviour damage) ||
                        damage.IsAlive())
        ...
        bool GetSightsResult(Vector3 threatPos, Vector3 headPos)
        {
            ...
            {
                if (
                    SeesObject(sensorSettings.sightsDistance, headPos) &&
                    IsObjectInSights(sensorSettings.sightsFOV, threatPos)
                )
                {
                    sightsResult = true;
                }
                else if (
                    SeesObject(sensorSettings.veryCloseRange, headPos) &&
                    !veryCloseAttract &&
                    !isAgonyOrEating
                )
                {
        ...
        public void PlaySoundOrDamageEvent(int type)
        {
            if (type == 0)
            {
                ...
                if (priorityThreat != null)
                {
                    ...
                    if (
                        behaviourSettings.damageThreat &&
                        IsObjectInSights(sensorSettings.attackFOV,
                        priorityThreat.position) &&
                        InDistance(sensorSettings.attackDistance,
                        priorityThreat.position)
                    )
                    {
                        if (priorityThreat == playerObject)
                            StartCoroutine(playerController
                                .ApplyKickback(new Vector3(damageSettings
                                        .damageKickback
                                        .x,
                                    damageSettings.damageKickback.y,
                                    0),
                                damageSettings.kickbackTime));
                        ...
                        )
                        {
                            damage
                                .ApplyDamage((int)
                                Random
                                    .Range(damageSettings.damageValue.x,
                                    damageSettings.damageValue.y));

                            if (!damage.IsAlive()) priorityThreat = null;
                        }
                    }
                }
            }
            ...
            {
                //Eat
                PlaySound(m_NPCSounds.EatingSound,
                m_NPCSoundsVolume.EatingVolume);
            }
        }
        ...
        void RotateTowards(Vector3 target)
        {
            if (target != Vector3.zero || target != null)
            {
                ...
                transform.rotation =
                    Quaternion
                        .SlerpUnclamped(transform.rotation,
                        lookRotation,
                        Time.deltaTime * aiSettings.agentRotationSpeed);
            }
        }
        ...
        WaypointGroup FindClosestWaypoints()
        {
            WaypointsData[] waypoints =
                (
                from w in FindObjectsOfType<WaypointGroup>()
                select new WaypointsData(w, 0)
                ).ToArray();
        ...
        HungerPoint FindClosestHungerPoint()
        {
            if (hungerPositions.Length > 0)
            {
                ...
                float closest_length =
                    CalculatePathLength(closest.transform.position);
                ...
                foreach (var point in hungerPositions)
                {
                    ...
                    if (
                        (
                        length = CalculatePathLength(point.transform.position)
                        ) >
                        closest_length
                    )
                    {
        ...
        float CalculatePathLength(Vector3 targetPosition)
        {
            ...
            {
                pathLength +=
                    Vector3.Distance(allWayPoints[i], allWayPoints[i + 1]);
            }
        ...
        void OnDrawGizmosSelected()
        {
            ...
            Vector3 leftRayDirection =
                Quaternion
                    .AngleAxis(-(sensorSettings.sightsFOV / 2), Vector3.up) *
                transform.forward;
            Vector3 rightRayDirection =
                Quaternion.AngleAxis(sensorSettings.sightsFOV / 2, Vector3.up) *
                transform.forward;

            if (Application.isPlaying && playerObject)
            {
                Gizmos.color =
                    IsObjectInSights(sensorSettings.sightsFOV,
                    playerObject.position) &&
                    SeesObject(sensorSettings.sightsDistance, playerHead)
                        ? Color.green
                        : Color.yellow;
            }
            else
            ...
            Gizmos
                ...
                leftRayDirection * sensorSettings.sightsDistance);
            Gizmos
                .DrawRay(transform.position,
                rightRayDirection * sensorSettings.sightsDistance);

            Vector3 leftAttackRay =
                Quaternion
                    .AngleAxis(-(sensorSettings.attackFOV / 2), Vector3.up) *
                transform.forward;
            Vector3 rightAttackRay =
                Quaternion.AngleAxis(sensorSettings.attackFOV / 2, Vector3.up) *
                transform.forward;
            Gizmos.color = Color.red;
            Gizmos
                .DrawRay(transform.position,
                leftAttackRay * sensorSettings.attackDistance);
            Gizmos
                .DrawRay(transform.position,
                rightAttackRay * sensorSettings.attackDistance);
            ...
            if (!gizmosSettings.soundReactionGizmos)
            {
                if (sleepBehaviour != StartingSleep.None)
                {
                    ...
                    Gizmos
                        .DrawWireSphere(transform.position,
                        sensorSettings.idleHearRange);
                }
                else
                {
                    Gizmos.color = Color.yellow;
            ...
            else
            {
                ...
                Gizmos
                    .DrawWireSphere(transform.position,
                    sensorSettings.soundReactRange);
            }
        }
        ...
        public void OnLoad(JToken token)
        {
            GetComponent<NavMeshAgent>()
                .Warp(token["position"].ToObject<Vector3>());
            ...
            {
                sleepBehaviour =
                    token["sleep_behaviour"].ToObject<StartingSleep>();
